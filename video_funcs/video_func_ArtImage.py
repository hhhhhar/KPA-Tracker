...
import sys
sys.path.append('../')
from dataset.dataset_Tracker_camera import SapienDataset_OMADNet
from model.OMAD_KPA_Tracker import OMAD_Net
...
from optimization.omad_optim import PoseEstimator, optimize_pose
...
def get_vis_resource(index, result, norm_kp, model, opt, device, complete_all_mesh_pts, fathest_sampler): # no_concat
    ...
    pose_estimator = PoseEstimator(model, opt.num_parts, opt.num_kp, init_base_r, init_base_t, 
                                   init_joint_state, norm_kp, gt_norm_joint_loc, 
                                   gt_norm_joint_axis, device=device, cate_id=opt.cate_id, debug=opt.show_omad, sort_part = sort_part)
    base_transform, pred_trans_joint_params_all, new_pred_joint_state = \
        optimize_pose(pose_estimator, pred_trans_part_kp, use_initial=opt.use_initial)
    ...
    for joint_idx in range(num_joints):
        ...
        if opt.cate_id != 4:
            relative_transform = RotateAnyAxis(start_point, end_point, new_pred_joint_state[joint_idx])
        elif opt.cate_id == 4:
            relative_transform = torch.cat([torch.cat([torch.eye(3, device=device),
                                                            (new_joint_axis*new_pred_joint_state[joint_idx]).unsqueeze(1)], dim=1),
                                            torch.tensor([[0., 0., 0., 1.]], device=device)], dim=0)
        pts_child_rts.append(relative_transform)

    pred_r_list = [base_transform[:3, :3].cpu().numpy()] + [
                    pts_child_rts[joint_idx].matmul(base_transform)[:3, :3].cpu().numpy()
                    for joint_idx in range(num_joints)]
    pred_t_list = [base_transform[:3, -1].cpu().numpy()] + [
                    pts_child_rts[joint_idx].matmul(base_transform)[:3, -1].cpu().numpy()
                    for joint_idx in range(num_joints)]
    pred_rt_list = [base_transform.cpu().numpy()] + [
                    pts_child_rts[joint_idx].matmul(base_transform).cpu().numpy()
                    for joint_idx in range(num_joints)]
